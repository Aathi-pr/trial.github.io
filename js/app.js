	...
	hamburger.addEventListener('click', () => {
		...
		spans.forEach((span, index) => {
			if (navLinks.classList.contains('active')) {
				if (index === 0) span.style.transform = 'rotate(45deg) traslate(5px, 5px)';
				if (index === 1) span.style.opacity = '0';
				if (index === 2) span.style.transform = 'rotate(-45deg) traslate(7px, -6px)';
			} else {
...
const animate = (element, keyframes, options) => {
    ...
    const observerForContainer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                observerForContainer.unobserve(entry.target);
            }
        });
    }, {
        threshold: 0.1
    ...
    observerForContainer.observe(orderContainer);
...
form.addEventListener('submit', async (e) => {
    ...
    if (isValid) {
        ...
        address: form.querySelector('[name="address"]').value,
                quantity: quantity // Get the dynamic quantity
            };
